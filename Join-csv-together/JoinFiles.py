...
def load_file(head, file_name, open_type = 'rb', delimiter = ","):
    ...
    if len(head) >= 1 and not isinstance(head, (int, long)): #if we actually have a header
        ...
        if l == (len(o_list)):
            ...
            o_list = o_list[l+1:]
    elif isinstance(l[1], (int, long)): #if the header is a number (start on row header)
        o_list = o_list[head:]
    else:
        1==1
    return o_list
...
for l in con:
    ...
    #    print(File_location)
      ...
      #  print(file_type)
        ...
        file_save = l[1]
      #  print(file_save)
    elif v == 'delimiter':           # if the control parameter has a delimiter
        delim = l[1]
      #  print(delim)
...
for filez in list_of_files:
    ...
    l_filez = load_file(header, filez, delimiter = delim)
    if len(l_filez) > 0:
        if len(replace_dict) > 0:
            for k, v in replace_dict.items():
                l_filez = replace_values(l_filez, k, v)
        with open(file_save, 'ab') as you_such_a_foo:
            csv_writer = csv.writer(you_such_a_foo,  delimiter = delim)
            for o in l_filez:
